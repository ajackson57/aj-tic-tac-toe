...
/***/ (function(module, exports, __webpack_require__) {
	...
	var uiGame = __webpack_require__(7);
	var uiAuth = __webpack_require__(12);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var currentBoard = __webpack_require__(8);
	var gameBoard = __webpack_require__(9);
	...
	var saveGameSuccess = function saveGameSuccess(data) {
	  $('#content').text('Save game succesfull');
	};

	var saveGameFailure = function saveGameFailure(error) {
	  $('#content').text('Error on save game: ' + error);
	};
	var getStatisticsSuccess = function getStatisticsSuccess(data) {
	  $('#content').text('Save game succesfull');
	};

	var getStatisticsFailure = function getStatisticsFailure(error) {
	  $('#content').text('Error on save game: ' + error);
	};

	var displayStatistics = function displayStatistics() {
	  var games = currentBoard.allGames;
	  $('#content').html('Game Statistics<br/>' + 'Total Games: ' + games.totalGames + '<br/>Completed Games: ' + games.completedGames + '<br/>Incomplete Games: ' + games.incompleteGames + '<br/>Games Played to Draw: ' + games.drawGames + '<br/>Games won by playerX: ' + games.xWon + '<br/>Games won by playerO: ' + games.oWon);
	};

	var displayGames = function displayGames() {
	  var allGames = currentBoard.allGames;
	  // for (games
	  var gameStatus = '';
	  for (var i = 0; i < allGames.games.length; i++) {
	    gameStatus += '<br/>' + allGames.games[i].id + '       ' + allGames.games[i].over;
	  }
	  $('.list-of-games').html(gameStatus);
	};

	var moveFeedback = function moveFeedback(feedback) {
	  $('#content').text(feedback);
	};

	var hideGameContent = function hideGameContent() {
	  $('#game-board').hide();
	  $('#get-statistics').hide();
	  $('#get-games').hide();
	  $('#get-game').hide();
	...
	module.exports = {
	  ...
	  getStatisticsSuccess: getStatisticsSuccess,
	  getStatisticsFailure: getStatisticsFailure,
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var getWinner = function getWinner() {
	  ...
	  if (this.cells[0] !== '' && this.cells[0] === this.cells[1] && this.cells[2]) {
	    ...
	    winner = this.cells[2];
	  } else if (this.cells.every(function (element) {
	    return element !== '';
	  })) {
	    winner = 'draw';
	  }
	...
	var updateGame = function updateGame(index, cellValue) {
	...
	var getGame = function getGame(gameId) {
	  currentBoard.currentGame = currentBoard.allGames.games.find(function (game) {
	...
	var getStatistics = function getStatistics() {
	  ...
	  if (currentBoard.allGames !== {}) {
	    ...
	    for (var i = 0; i < currentBoard.allGames.games.length; i++) {
	      var game = new GameFromGame(currentBoard.allGames.games[i]);
	      if (game.over === true) {
	...
	var Games = function Games(games) {
	  this.games = games;
	  this.totalGames = 0;
	...
	var GameFromGame = function GameFromGame(game) {
	  this.id = game.id;
	...
	var inistializeGame = function inistializeGame(data) {
	  'In initializeGame with data: ' + data;
	  currentBoard.currentGame = new GameFromGame(data.game);
	  var board = $('#game-board > tbody');
	  // ('The board ', board[0].rows)
	  //  ('Intialize Game')
	  var i = 0;
	  for (var rowIndex = 0; rowIndex < board[0].rows.length; rowIndex++) {
	    for (var colIndex = 0; colIndex < board[0].rows[rowIndex].children.length; colIndex++) {
	...
	var updateGameFromDom = function updateGameFromDom() {
	  // #game-board > tbody > tr:nth-child(1) > td:nth-child(1)
	  // #game-board > tbody > tr:nth-child(2) > td:nth-child(3)
	  // #game-board > tbody > tr:nth-child(1)
	  var board = $('#game-board > tbody');
	...
	module.exports = {
	  inistializeGame: inistializeGame,
	  updateGameFromDom: updateGameFromDom,
	  updateGame: updateGame,
	  getStatistics: getStatistics,
	  getWinner: getWinner,
	  togglePlayer: togglePlayer,
	  Game: Game,
	  GameFromGame: GameFromGame,
	  Games: Games,
	  getGame: getGame
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var update = function update(gameId, data) {
	  return $.ajax({
	    url: config.apiOrigin + '/games/' + gameId,
	    method: 'PATCH',
	    headers: {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var store = __webpack_require__(11);
	var uiGames = __webpack_require__(7);
	...
	var hideAuthContent = function hideAuthContent() {
	  $('#sign-up').hide();
	  $('#sign-in').hide();
	  $('#sign-out').hide();
	  $('#change-password').hide();
	...
	module.exports = {
	  ...
	  hideAuthContent: hideAuthContent
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onCellClick = function onCellClick(event) {
	  ...
	  var mapArray = [[0, 1, 2], [3, 4, 5], [6, 7, 8]];
	  ...
	  var results = game.currentGame.updateGame(mapArray[rowIndex][colIndex], event.target.textContent);
	...
	var onGetStatistics = function onGetStatistics(event) {
	  event.preventDefault();
	  if (jQuery.isEmptyObject(store.user)) {
	    $('#content').text('Please sign in.');
	    return;
	...
	var onGetGames = function onGetGames(event) {
	  ...
	  api.index().then(ui.getGamesSuccess).catch(ui.getGamesFailure);
	};
	var onNewGame = function onNewGame(event) {
	  ...
	  api.create().then(ui.newGameSuccess).catch(ui.newGameFailure);
	};
	var onGetGame = function onGetGame(event) {
	  ...
	  var gameForm = getFormFields(event.target);
	  event.preventDefault();
	  var gameId = gameForm.game.id;
	  api.show(gameId).then(ui.getGameSuccess).catch(ui.getGameFailure);
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onSignInMenu = function onSignInMenu(event) {
	  event.preventDefault();
	  $('#sign-in').show();
	};
	var onSignOutMenu = function onSignOutMenu(event) {
	  event.preventDefault();
	  $('#sign-out').trigger('submit');
	};
	var onSignUpMenu = function onSignUpMenu(event) {
	  event.preventDefault();
	  $('#sign-up').show();
	};
	var onChangePasswordMenu = function onChangePasswordMenu(event) {
	  event.preventDefault();
